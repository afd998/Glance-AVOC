...
const LayoutContent: React.FC<LayoutProps> = ({ children }) => {
  ...
  const {
    ...
    pixelsPerMin,
    rowHeightPx,
    startHour: profileStartHour,
    endHour: profileEndHour,
  } = useProfile();
  ...
  const { showEventAssignments, setShowEventAssignments } =
    useEventAssignments();
  ...
  const generateBreadcrumbs = (): BreadcrumbItem[] => {
    ...
    pathSegments.forEach((segment, index) => {
      ...
      if (segment === "faculty") {
        ...
        /^\d+$/.test(segment) &&
        pathSegments[index - 1] === "faculty"
      ) {
        // This is a faculty ID - use the faculty name if available
        if (facultyMember) {
          label =
            facultyMember.kelloggdirectory_name ||
            facultyMember.twentyfivelive_name ||
            `Faculty ${segment}`;
        } else {
  ...
  useEffect(() => {
    if (currentBackground && currentBackground !== "offwhite") {
      ...
      img.onerror = () =>
        console.error("Background image failed to load:", currentBackground);
  ...
  useEffect(() => {
    ...
    if (
      typeof pixelsPerMin === "number" &&
      pixelsPerMin > 0 &&
      pixelsPerMin !== basePixelsPerMinute
    ) {
      setBasePixelsPerMinute(pixelsPerMin);
    }
    if (
      typeof rowHeightPx === "number" &&
      rowHeightPx > 0 &&
      rowHeightPx !== baseRowHeightPx
    ) {
  ...
  const previousProfileHoursRef = useRef<{ start: number; end: number } | null>(
    null
  );

  useEffect(() => {
    if (
      typeof profileStartHour !== "number" ||
      typeof profileEndHour !== "number"
    )
      return;
    const prev = previousProfileHoursRef.current;
    if (
      prev &&
      prev.start === profileStartHour &&
      prev.end === profileEndHour
    ) {
      return;
    }
    previousProfileHoursRef.current = {
      start: profileStartHour,
      end: profileEndHour,
    };
  ...
  return (
    <div className="min-h-screen w-full flex relative" style={{ zIndex: 2 }}>
      ...
      {currentBackground !== "none" && (
        <div
          ...
          style={{
            zIndex: 0,
            backgroundImage:
              currentBackground && currentBackground !== "offwhite"
            ...
            backgroundSize:
              currentBackground === "jaobscenter.jpeg"
                ...
                : "120% 120%",
            backgroundRepeat:
              currentBackground === "offwhite" ? "repeat" : "no-repeat",
      ...
      {currentBackground !== "none" && (
        <div
          ...
          style={{
            zIndex: 1,
            background:
              "radial-gradient(circle at center, rgba(255, 255, 255, 1.0) 0%, rgba(255, 255, 250, 0.9) 20%, rgba(255, 250, 240, 0.7) 40%, rgba(255, 240, 220, 0.5) 60%, transparent 80%)",
            mixBlendMode: "screen",
          }}
        />
      )}
      ...
      {showAvocHomeText && (
        <div className="fixed inset-0 -z-5 flex items-center justify-center pointer-events-none">
          <div className="text-center">
            <div
              className="text-[20rem] sm:text-[25rem] md:text-[30rem] lg:text-[35rem] xl:text-[40rem] font-black text-white/20 select-none"
              style={{
      ...
      <SidebarInset className="flex-1 h-screen w-full z-3 overflow-x-hidden flex flex-col">
